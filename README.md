...
src/
  ...
  kernel/      store (event log + atomic budget CAS) · policy (PDP) ·
               tools (registry) · engine (the supervisor-tree turn loop)
  demo/        the research-&-content company · runner · JSON snapshot
app/           Next.js dashboard (Boardroom / budget board / Review Center / feed)
               + /api/state
...
PDP + approval gates, ScriptedAdapter + AnthropicAdapter, the $0 demo, the
Next.js dashboard, and the test suite (25 tests, green).

**Next (per the plan):** persist the log to Neon + run the loop as Inngest durable
steps (§11/§12); review-as-gate; dynamic hiring (`open_requisition`); MCP/Nango
connectors; the flow-canvas workspace. Tracked in `ULTRAPLAN.md` §Appendix.

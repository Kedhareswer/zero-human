...
async function buildGatedOrg() {
  const store = new InMemoryStore();
  const meta = { orgId: "org", actor: "human" };
  await store.append({ type: "OrgCreated", goal: { title: "Ship it" }, aggregateCapCents: 1000 }, meta);
  await store.append({ type: "RoleDefined", roleId: "r", name: "Eng", authorities: auth({}) }, meta);
  await store.append({ type: "PositionCreated", positionId: "eng", roleId: "r", parentId: null }, meta);
  await store.append({ type: "AgentHired", positionId: "eng", agentId: "a", name: "Dev", provider: "claude", model: "claude-opus-4-8", systemPrompt: "[pos:eng] ship", monthlyCapCents: 1000, capabilities: ["deploy"] }, meta);
  await store.append({ type: "OrgStateChanged", state: "RUNNING" }, meta);
  await store.initBudget("eng", 1000);
...
describe("human approval gates on irreversible actions", () => {
  it("a granted approval lets the gated tool run", async () => {
    const { store, script, registry, positionOf } = await buildGatedOrg();
